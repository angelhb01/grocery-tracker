...
export default function Signup() {
  ...
  async function signUpWithEmail() {
    const isValid = /^(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*.])(?=.{8,})/.test(password);
    if (!isValid) {
      Alert.alert("Password does not meet the requirements. Please try again.");
      return;
    ...
    try {
      ...
      if (error) {
        ...
        //router.replace({ pathname: '/(screens)/profileEdit' });
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.container} className='m-6 border-white shadow-sm rounded-md bg-white'>
        <View className="flex-col justify-center items-center gap-3">
          ...
          <Text className="text-2xl text-center font-bold">Create Your Account</Text>
          <Text className="text-1xl text-center text-slate-600">Join us and start tracking your groceries today.</Text>
        </View>
            ...
            <View className='flex-1 gap-3'>
              ...
              <View className='flex-col gap-2'>
                <View className='flex-row gap-2 justify-start'>
                  ...
                  <Text className='text-[0.8rem] flex-1'>At least 8 characters</Text>
                </View>
                <View className='flex-row gap-2 justify-start'>
                  <AntDesign name="check-circle" size={15} color="green" />
                ...
                <View className='flex-row gap-2 justify-start'>
                  <AntDesign name="check-circle" size={15} color="green" />
                  <Text className='text-[0.8rem] flex-1'>1 special character</Text>
                </View>
              </View>
            </View>
          </View>
        ...
        <View className='flex-row my-2'>
          ...
          <Link href={{ pathname: '/(authentication)/login' }} className='text-green-700'>Login here</Link>
        </View>
      </View>
